      ...
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <li class="order-info">
            ...
            <div class="order-time">Create time: {{item[0].order_time | dateFormat}}</div>
          </li>
          <li class="header">
          ...
          <li class="product-list" v-for="(product,i) in item" :key="i">
            <div class="pro-img">
              <router-link :to="{ path: '/goods/details', query: {productID:product.product_id} }">
                <img :src="$resource + product.product_picture" />
              </router-link>
            </div>
            <div class="pro-name">
              <router-link
            ...
            <div class="pro-price">{{product.product_price}}yen</div>
            <div class="pro-num">{{product.product_num}}</div>
            <div class="pro-total pro-total-in">{{product.product_price*product.product_num}}yen</div>
          </li>
        </ul>
        <div class="order-bar">
          <div class="order-bar-left">
            <span class="order-total">
              <span class="order-total-num">{{total[index].totalNum}}</span> in total
            </span>
          </div>
          <div class="order-bar-right">
            <span>
              <span class="total-price-title">Total amount: </span>
              <span class="total-price">{{total[index].totalPrice}}yen</span>
            </span>
          </div>
          <!-- 订单列表END -->
        </div>
